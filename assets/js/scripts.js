function onResize(){
	$('.blog-post-con').height('auto');
	var elementHeights = $('.blog-post-con').map(function() {
		return $(this).height();
	}).get();
	var maxHeight = Math.max.apply(null, elementHeights);
	$('.blog-post-con').height(maxHeight);
}
...
//HOME PAGE ICONS -------------------------------------------------->
	...
	$('.menu li').click(function(e) {
		...
		var urlhash = buttonInfo.replace('W','');
	...
	if (window.location.href.indexOf('#ConsumerPackagedGoods') > -1) {
	...
	if (window.location.href.indexOf('#BuildingSupply') > -1) {
